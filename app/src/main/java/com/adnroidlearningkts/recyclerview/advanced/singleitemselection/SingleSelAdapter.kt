...
class SingleSelAdapter(val onSingleItemSelection: (Employee) -> Unit):
    ListAdapter<Employee, SingleSelAdapter.ViewHolder>(DIFF_CALLBACK) {
    ...
    inner class ViewHolder(val binding: RvSingleSelItemBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(employee: Employee) {
            ...
            binding.advRvSingleSelLayout.setOnClickListener {
                onSingleItemSelection(employee)
            }
        }
    }
}